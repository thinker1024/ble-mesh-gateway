...
unsigned char uart_Init(unsigned short uartCLKdiv, unsigned char bwpc,unsigned char en_rx_irq,unsigned char en_tx_irq,enum HARDWARECONTROL hdwC){
	...
	#if(UART_CONTINUE_DELAY_EN)
	baudrate_set = (CLOCK_SYS_CLOCK_HZ / (uartCLKdiv + 1)) / (bwpc + 1);
	#endif
...
unsigned char uart_print(const char* p){

	uart_data_t T_txdata_print;
	unsigned int len = strlen(p);
	T_txdata_print.len = len;
	memcpy(T_txdata_print.data, p, len);
    ...
    uart_set_tx_busy_flag();
    memcpy(tx_buff, (u8 *)&T_txdata_print, len + 4);
    write_reg16(0x800504, (unsigned short)((unsigned int)tx_buff)); //packet data, start address is sendBuff+1
...
//        return 0;
//    }
	T_txdata_print.data[0] = p;
	T_txdata_print.data[1] = '\0';
    ...
    uart_set_tx_busy_flag();
    memcpy(tx_buff, (u8 *)&T_txdata_print, 6);
    write_reg16(0x800504, (unsigned short)((unsigned int)tx_buff)); //packet data, start address is sendBuff+1
