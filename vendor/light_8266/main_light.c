...
//		uart_print("device address is set ok!!!\r\n");
        mesh_node_init();
        device_status_update();
        cfg_led_event(LED_EVENT_FLASH_1HZ_4S);
...
int light_notify(u8 *p, u8 len, u8* p_src){
    ...
    if(slave_link_connected && pair_login_ok){
        ...
        if (is_add_packet_buf_ready()){
            rf_link_add_tx_packet ((u32)(&pkt_light_notify));
        }else{
            ret = -1;
        }
        irq_restore(r);
...
void light_sim_light2light(void){
	...
	if(clock_time_exceed(last_time, 5000*1000)){
        if(!is_tx_cmd_busy()){   // it must be used, if want enough bridges(default is BRIDGE_MAX_CNT 8).
    		...
    		static u8 cnt = 0;
    		u8 op_para[16] = {0};
    		op_para[0] = LGT_CMD_LIGHT_ONOFF;  //changed from ONOFF to OFF
...
//        	light_adjust_W(PMW_MAX_TICK_BASE,255);
		...
		if((rx_len == 13) && (0 == myStrCmp((char *)T_rxdata_user.data, "BT+WOFF", 7))){
        	uart_print("OFF,ok\r\n");
        	ex_param[0] = 0;
        	gateway_light2light(LGT_CMD_LIGHT_W, ex_param, 0xFFFF);
        }
        if((rx_len > 16 && rx_len < 20) && (0 == myStrCmp((char *)T_rxdata_user.data, "BT+OFF+LID", 10))){
        ...
        if((rx_len > 17 && rx_len < 21) && (0 == myStrCmp((char *)T_rxdata_user.data, "BT+WOFF+LID", 11))){
        	xdata = atoi(((char *)T_rxdata_user.data) + 11);
        	if(xdata >= 0 && xdata <=255){
            	uart_print("OFF,ok\r\n");
            	ex_param[0] = 0;
            	gateway_light2light(LGT_CMD_LIGHT_W, ex_param, xdata);
        	}
        }
        if((rx_len > 16 && rx_len < 20) && (0 == myStrCmp((char *)T_rxdata_user.data, "BT+OFF+GID", 10))){
        ...
        if((rx_len > 17 && rx_len < 21) && (0 == myStrCmp((char *)T_rxdata_user.data, "BT+WOFF+GID", 11))){
		...
		if((rx_len == 12) && (0 == myStrCmp((char *)T_rxdata_user.data, "BT+WON", 6))){
        	uart_print("ON,ok\r\n");
        	ex_param[0] = 1;
        	gateway_light2light(LGT_CMD_LIGHT_W, ex_param, 0xFFFF);
        }
        if((rx_len > 15 && rx_len < 19) && (0 == myStrCmp((char *)T_rxdata_user.data, "BT+ON+LID", 9))){
        ...
        if((rx_len > 16 && rx_len < 20) && (0 == myStrCmp((char *)T_rxdata_user.data, "BT+WON+LID", 10))){
        	xdata = atoi(((char *)T_rxdata_user.data) + 10);
        	if(xdata >= 0 && xdata <=255){
            	uart_print("ON,ok\r\n");
            	ex_param[0] = 0x01;
        ...
        if((rx_len > 15 && rx_len < 19) && (0 == myStrCmp((char *)T_rxdata_user.data, "BT+ON+GID", 9))){
        ...
        if((rx_len > 16 && rx_len < 20) && (0 == myStrCmp((char *)T_rxdata_user.data, "BT+WON+GID", 10))){
        	xdata = atoi(((char *)T_rxdata_user.data) + 10) + 0x8000;
        	if(xdata >= 0x8000 && xdata <= 0xFFFE){
            	uart_print("ON,ok\r\n");
            	ex_param[0] = 0x01;
        ...
        if((rx_len > 14) && (rx_len <19) && (0 == myStrCmp((char *)T_rxdata_user.data, "BT+WOFFD", 8))){
        	uart_print("OFF delay,ok\r\n");
        ...
        if(rx_len > 13 && rx_len < 18 && (0 == myStrCmp((char *)T_rxdata_user.data, "BT+WOND", 7))){
        	uart_print("ON delay,ok\r\n");
        	ex_param[0] = 0x01;
        	xdata = atoi(((char *)T_rxdata_user.data) + 7);
        	ex_param[1] = xdata & 0x00ff;
        ...
        if(rx_len >19 && rx_len <27 && (0 == myStrCmp((char *)T_rxdata_user.data, "BT+LID+SETLUM", 13))){
        	uart_print("SET lum,ok\r\n");
        	for(int i=0;i<(rx_len-19);i++)
        	{
        ...
        if(rx_len >16 && rx_len <20 && (0 == myStrCmp((char *)T_rxdata_user.data, "BT+SETWLUM", 10))){
        	...
        	for(int i=0;i<(rx_len-18);i++)
        	{
        		if(T_rxdata_user.data[12+i] == ','){
        			...
        			if(count == 1){
        				...
        				ex_param[2] = atoi(rgb + len_r + 1);
        				len_rg = strlen(rgb);
        			}
        		}
        		rgb[i] = T_rxdata_user.data[12+i];
        	}
        	ex_param[3] = atoi(rgb + 1 + len_rg);
        	gateway_light2light(LGT_CMD_SET_RGB_VALUE, ex_param, 0xFFFF);
        }
        if(rx_len >22 && rx_len <39 && (0 == myStrCmp((char *)T_rxdata_user.data, "BT+LID+SETRGBLUM", 16))){
        	uart_print("SET RGB lum,ok\r\n");
        	ex_param[0] = 0x04;
        	u8 count = 0;
        	u8 len_r = 0;
        	...
        	for(int i=0;i<(rx_len-22);i++)
        	{
        		if(T_rxdata_user.data[16+i] == ','){
        			...
        			if(count == 1){
        				...
        				ex_param[3] = atoi(rgb + len_rg + 1);
        				len_rgb = strlen(rgb);
        			}
        		}
        		rgb[i] = T_rxdata_user.data[16+i];
        	}
        	xdata = atoi(rgb + len_rgb + 1);
        	gateway_light2light(LGT_CMD_SET_RGB_VALUE, ex_param, xdata);
        }
        if(rx_len >22 && rx_len <39 && (0 == myStrCmp((char *)T_rxdata_user.data, "BT+GID+SETRGBLUM", 16))){
        	uart_print("SET RGB lum,ok\r\n");
        	ex_param[0] = 0x04;
        	u8 count = 0;
        	u8 len_r = 0;
        ...
        if(rx_len >17 && rx_len <25 && (0 == myStrCmp((char *)T_rxdata_user.data, "BT+LID+ADDG", 11))){
        	...
        	for(int i=0;i<(rx_len-17);i++)
        	{
        		...
        		lid[i] = T_rxdata_user.data[11+i];
        	}
        	xdata = atoi(lid + 1 + len_lid) + 0x8000;
        	ex_param[1] = xdata & 0x00ff;
        	ex_param[2] = (xdata & 0xff00) >> 8;
...
void rf_link_data_callback (u8 *p)
{
    ...
    if(op_cmd_len == op_type_1){
        ...
        if(vendor_id == VENDOR_ID){
        	if(op == LGT_CMD_LIGHT_ONOFF || op == LGT_CMD_LIGHT_W){
...
//		memset(temp, '\0', 10);
//		int xnum = (*(ppp->val + 3)) + (*(ppp->val + 4) << 8);
//		itoa(xnum, temp, 16);
//		uart_print("Light ID:");
//		uart_print((const char *)temp);
//		uart_print("\r\n");
//	    cfg_led_event(LED_EVENT_FLASH_4HZ_3T);
...
#if DEMO_HARDCODE
	extern u8 rf_link_add_dev_addr(u16 deviceaddress);
	rf_link_add_dev_addr(DEFAULT_LIGHT_ID);
#endif
}
...
//    CLK32M_UART9600;
    ...
    i2c_pin_initial(I2C_HW_SDA, I2C_HW_SCL);
    #if I2C_MASTER_MODE
